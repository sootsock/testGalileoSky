...
export function validateSchema(schema: unknown): SchemaValidationResult {
  ...
  const s = schema as Partial<FormSchema> & Record<string, unknown>;

  if (!s.title || typeof s.title !== "string") errors.push("title must be a non-empty string");
  if (!Array.isArray(s.fields)) errors.push("fields must be an array");
  if (!s.id || typeof s.id !== "string") errors.push("id must be a string");
